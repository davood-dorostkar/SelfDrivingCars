...
class PathOptimizer:
    ...
    def sample_spiral(self, p):
        ...
        c = (9.0 * p[0] - 45.0 * p[1] / 2.0 + 18.0 * p[2] - 9.0 * p[3] / 2.0) / p[
            4
        ] ** 2
        d = (
            -(
        ...
        x_points = scipy.integrate.cumulative_trapezoid(
            cos(t_points), s_points, initial=0
        )
        y_points = scipy.integrate.cumulative_trapezoid(
            sin(t_points), s_points, initial=0
        )
    ...
    def ftf(self, p):
        t2 = (
            ...
            - p[0] * p[4]
            + p[4]
            * (p[0] * (1.1e1 / 2.0) - p[1] * 9.0 + p[2] * (9.0 / 2.0) - p[3])
            * (1.0 / 2.0)
            + p[4]
            * (
                p[0] * (9.0 / 2.0)
