...
class CarlaGame(object):
    ...
    def _initialize_game(self):
        ...
        if self._city_name is not None:
            ...
            extra_width = int(
                (WINDOW_HEIGHT / float(self._map_shape[0])) * self._map_shape[1]
            )
        ...
        if self._timer.elapsed_seconds_since_lap() > 1.0:
            if self._city_name is not None:
                ...
                self._print_player_measurements_map(
                    measurements.player_measurements, map_position, lane_orientation
                )
            else:
    ...
    def _print_player_measurements_map(
        self, player_measurements, map_position, lane_orientation
    ):
    ...
    def _on_render(self):
        ...
        if self._map_view is not None:
            ...
            h_pos = int(
                self._position[1] * (new_window_width / float(self._map_shape[1]))
            )
...
def main():
    ...
    argparser.add_argument(
        "-m", "--map", action="store_true", help="plot the map of the current city"
    )
